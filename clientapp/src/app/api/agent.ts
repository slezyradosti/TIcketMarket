...
const Events = {
  ...
  getSellersOne: (id: string) => requests.get<Event>(`/event/my-events/${id}`),
  getOne: (id: string) => requests.get<EventExtendedDto>(`/event/${id}`),
...
const EventCategories = {
  ...
  getSellersOne: (id: string) =>
    requests.get<EventCategory>(`/EventCategory/${id}`),
  createOne: (eventCategory: EventCategory) =>
    requests.post<void>("/EventCategory", eventCategory),
  editOne: (eventCategory: EventCategory) =>
    requests.put<void>(`/EventCategory/${eventCategory.id}`, eventCategory),
...
const EventTables = {
  ...
  createOne: (eventTable: EventTable) =>
    requests.post<void>("/EventTable", eventTable),
  editOne: (eventTable: EventTable) =>
    requests.put<void>(`/EventTable/${eventTable.id}`, eventTable),
...
const EventTypes = {
  ...
  createOne: (eventType: EventType) =>
    requests.post<void>("/EventType", eventType),
  editOne: (eventType: EventType) =>
    requests.put<void>(`/EventType/${eventType.id}`, eventType),
...
const TableEvents = {
  list: (eventId: string) =>
    requests.get<TableEvent[]>(`/TableEvent/list/${eventId}`),
  getOne: (id: string) => requests.get<TableEvent>(`/TableEvent/${id}`),
  createOne: (tableEvent: TableEvent) =>
    requests.post<void>("/TableEvent", tableEvent),
  editOne: (tableEvent: TableEvent) =>
    requests.put<void>(`/TableEvent/${tableEvent.id}`, tableEvent),
  deleteOne: (id: string) => requests.delete<void>(`/TableEvent/${id}`),
};

const Tickets = {
  // customersList: () => requests.get<Ticket[]>('/Ticket/my-tickets'),
  availableTicketList: (eventId: string) =>
    requests.get<Ticket[]>(`/Ticket/available-tickets/${eventId}`),
  ...
  generateTickets: (ticket: Ticket, amount: number) =>
    requests.post<void>(
      `/Ticket/generate-tickets?ticket-amount=${amount}`,
      ticket
    ),
  getEventTicketAmount: (eventId: string) =>
    requests.get<EventTicketsAmountDto>(
      `/Ticket/event-ticket-amount/${eventId}`
    ),
  applyDiscount: (applyDiscountDto: ApplyDiscountDto) =>
    requests.put<void>("/Ticket/apply-discount", applyDiscountDto),
  removeDiscount: (ticketId: string) =>
    requests.put<void>(`/Ticket/remove-discount/${ticketId}`, {}),
  getTicketToBuy: (eventId: string, ticketTypeId: string) =>
    requests.get<Ticket>(
      `/Ticket/ticket-to-buy?event-id=${eventId}&ticket-type-id=${ticketTypeId}`
    ),
  purchaseTicket: (ticketId: string) =>
    requests.post<void>(`/Ticket/purchase-ticket/${ticketId}`, {}),
  detailedTicketList: (eventId: string) =>
    requests.get<Ticket[]>(`/Ticket/events-tickets/${eventId}`),
};

const TicketDiscounts = {
  getSellersList: () =>
    requests.get<TicketDiscount[]>(`/TicketDiscount/my-discounts`),
  getSellersOne: (id: string) =>
    requests.get<TicketDiscount>(`/TicketDiscount/${id}`),
  createSellersOne: (ticketDiscount: TicketDiscount) =>
    requests.post<void>("/TicketDiscount", ticketDiscount),
  editSellersOne: (ticketDiscount: TicketDiscount) =>
    requests.put<void>(`/TicketDiscount/${ticketDiscount.id}`, ticketDiscount),
  deleteSellersOne: (id: string) =>
    requests.delete<void>(`/TicketDiscount/${id}`),
};

const TicketOrders = {
  getCustomersList: () =>
    requests.get<TicketOrder[]>(`/TicketOrder/my-ticket-orders`),
  getCustomersOne: (id: string) =>
    requests.get<TicketOrder>(`/TicketOrder/${id}`),
  createOne: (ticketOrder: TicketOrder) =>
    requests.post<void>("/TicketOrder", ticketOrder),
  editOne: (ticketOrder: TicketOrder) =>
    requests.put<void>(`/TicketOrder/${ticketOrder.id}`, ticketOrder),
...
const TicketTypes = {
  ...
  createSellersOne: (ticketType: TicketType) =>
    requests.post<void>("/TicketType", ticketType),
  editOne: (ticketType: TicketType) =>
    requests.put<void>(`/TicketType/${ticketType.id}`, ticketType),
